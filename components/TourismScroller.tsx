          ...
          <Link key={d.slug} href={`/tourism/${d.slug}`} className="no-underline group flex-shrink-0">
            <div className="w-72 md:w-80 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
              <div className="relative h-40 md:h-48">
                <SafeImage src={d.heroImage} alt={d.name} mobileQuery={`${d.name} Nigeria`} className="absolute inset-0 w-full h-full object-cover" fallbackSrc="https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=1200&auto=format&fit=crop&q=80" />
